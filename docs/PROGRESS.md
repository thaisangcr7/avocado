...
- **Shell scrolling** · The viewport-height collapse that survived the pinning commit is closed, and it was never the fixed shell: the flex row inside it was a silent scroll container. `DocumentPanel` sat in the Library's scroller inside an auto-height wrapper, so its `h-full` resolved to auto and it laid out at full content height with a second, never-scrolling scroller inside; the `sr-only` file input down in that layout is absolutely positioned, and its nearest positioned ancestor was the row rather than the rail, so nothing clipped it and the row's scroll height reached 2014px against a 664px box. `overflow-hidden` still scrolls programmatically, so `scrollIntoView` — which scrolls *every* scrollable ancestor — slid the whole app up with no scrollbar to show why. The rail is one scroll owner again, the row and main are `overflow-clip` (not a scroll container at all, so no future overflow can move the shell), and the chat scrolls its own host by hand, instantly — a smooth scroll was restarted by every token and only landed after the stream ended.
