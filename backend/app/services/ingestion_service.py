...
class IngestionService:
    ...
    async def _parse(self, document: Document, data: bytes) -> ParsedDocument:
        ...
        # A PDF whose pages yield no text is a scan. Falling back to vision
        # recovers it instead of storing an empty document.
        if document.doc_type is DocumentType.PDF and parsed.metadata.get("likely_scanned"):
            log.info("pdf_appears_scanned", document_id=str(document.id))
            parsed.metadata["ocr_fallback"] = "attempted"
